        ...
        @forelse($wordLists as $list)
            <div class="shadow-md border border-gray-100 rounded-xl flex flex-col hover:shadow-xl mr-5 ml-5 bg-white">
                <a href="{{ route('open-word-list', $list['slug']) }}">
                    <img class="image-container mb-5 rounded-t-xl" src="{{ $list['image'] }}" alt="{{ $list['title'] }}">
                    <h2 class="h-20 pl-4 pr-4 word-list-description">{{ $list['title'] }}</h2>
                    <div class="word-list-stat flex-grow pl-4 mt-7 text-gray-400 mb-5">
